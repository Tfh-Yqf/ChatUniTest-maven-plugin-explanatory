...
## 任务1.1.1.3服务化改造：当前ChatUniTest主要是以客户端工具的形式来使用，我们希望可以通过Web服务的形式对外提供服务，这样客户端可以是各种IDE插件，客户端只需要负责用户交互，测试用例生成的主体功能则由服务端完成。建议用时5天，方案分满分50（仅有方案未实现不得分）、完成分满分50，总计100分。
