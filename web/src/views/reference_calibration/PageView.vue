  ...
  <div class="content-container">
    <div class="input-wrapper">
      <div class="title-bar">
          ...
          <div class="title-text">基准点标定</div>
      ...
      <div class="input-block" v-for="p in navList" :key="p.name">
        <div class="input-title-bar">
          <img src="@/assets/img/left-bar/position-icon.png" class="input-title-left-icon">
          <div class="input-title-text" v-if="$data[p.type] == 1" @click="changeFolderStatus(p.type, 0)">
            <div class="input-title-name">{{ p.name }}</div>
            <div class="triangle-down"></div>
          </div>

          <div class="input-title-text" v-if="$data[p.type] == 0" @click="changeFolderStatus(p.type, 1)">
            <div class="input-title-name">{{ p.name }}</div>
    ...
    <div class="output-wrapper" v-if="$store.state.position.showResult">
...
import ParamsConfig from "./params-config/PageView.vue";
...
export default {
  ...
  data: () =>{
    return {
      navList: [
        {name: '标定-参数配置', type: 'config', component: ParamsConfig},
      ],
      config: 0,
    }
  },
